...
class CCrmStartPageComponent extends CBitrixComponent
{
	...
	protected function prepareCounters()
	{
		...
		//Disabled because of there is no opportunity to reset default widget filter
		/*
	...
	public function executeComponent()
	{
		...
		$filterOptions = new Main\UI\Filter\Options($this->widgetGuid, array());
		$filterFields = $filterOptions->getFilter(
			array(
					array('id' => 'RESPONSIBLE_ID'),
					array('id' => 'PERIOD', 'type' => 'date')
			)
		);
		Filter::convertPeriodFromDateType($filterFields, 'PERIOD');
		Filter::sanitizeParams($filterFields);
		$this->commonFilter = new Filter(Filter::internalizeParams($filterFields));
		//endregion
