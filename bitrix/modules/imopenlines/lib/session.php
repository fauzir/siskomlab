...
class Session
{
	...
	public function start($params)
	{
		...
		if (!$this->prepareUserChat($params))
		{
			return false;
		}
	...
	private function prepareUserChat($params, $count = 0)
	{
		...
		if ($result)
		{
			...
			{
				usleep(500);
				$this->prepareUserChat($params, ++$count);
			}
			else
			{
				return false;
		...
		if ($this->chat->isNowCreated() && $this->config['AGREEMENT_MESSAGE'] == 'Y' && $enableSystemMessage)
		{
			...
			\CIMChat::AddMessage(Array(
				...
				"MESSAGE" => Loc::getMessage('IMOL_SESSION_AGREEMENT_MESSAGE', Array(
					'#LINK_START#' => '[URL='.\Bitrix\ImOpenLines\Common::getAgreementLink($this->config['AGREEMENT_ID'], $fields)."]",
					'#LINK_END#' => '[/URL]'
				)),
				"SYSTEM" => 'Y',
				"IMPORTANT_CONNECTOR" => 'Y',
