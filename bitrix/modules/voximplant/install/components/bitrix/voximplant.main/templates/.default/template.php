		...
		<div class="tel-history-block">
			...
			<? foreach ($arResult['STATISTICS'] as $statLine): ?>
				<div class="tel-history-block-info <?=$firstLine ? 'tel-history-block-info-current' : ''?>">
					<strong><?=GetMessage('TELEPHONY_MONTH_'.sprintf('%\'02u', $statLine['MONTH']))?> <?=$statLine['YEAR']?></strong> &mdash; <?=htmlspecialcharsbx($statLine["DURATION_FORMATTED"])?> <span class="tel-history-text-right"><?=htmlspecialcharsbx($statLine['COST_FORMATTED'])?></span>
				</div>
				<? $firstLine = false; ?>
			<? endforeach;?>
