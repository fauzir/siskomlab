...
class MobileMenu extends \CBitrixComponent
{
	...
	public function executeComponent()
	{
		...
		else
		{
			...
			$menu = include(\Bitrix\Main\Application::getDocumentRoot() . $this->getPath() . "/.mobile_menu.php");
			$events = \Bitrix\Main\EventManager::getInstance()->findEventHandlers("mobile", "onMobileMenuStructureBuilt");
			if (count($events) > 0)
			{
				$menu = ExecuteModuleEventEx($events[0], array($menu));
			}

			usort($menu, 'MobileMenu::sort');
			$arResult["MENU"] = $menu;
