...
final class Filter
{
	...
	public static function prepareFilter($filter)
	{
		...
		if(array_key_exists('RESPONSIBLE_ID', $filter))
		{
			...
			{
				$name = \Bitrix\Tasks\UI\Task::getUserInfo($userId, true);
				$filter['RESPONSIBLE_ID_label'][$k] = $name['NAME'];
			}
		}
		if(array_key_exists('CREATED_BY', $filter))
		{
			...
			{
				$name = \Bitrix\Tasks\UI\Task::getUserInfo($userId, true);
				$filter['CREATED_BY_label'][$k] = $name['NAME'];
			}
		}
		if(array_key_exists('AUDITOR', $filter))
		{
			...
			{
				$name = \Bitrix\Tasks\UI\Task::getUserInfo($userId, true);
				$filter['AUDITOR_label'][$k] = $name['NAME'];
			}
		}
		if(array_key_exists('ACCOMPLICE', $filter))
		{
			...
			{
				$name = \Bitrix\Tasks\UI\Task::getUserInfo($userId, true);
				$filter['ACCOMPLICE_label'][$k] = $name['NAME'];
			}
		}
