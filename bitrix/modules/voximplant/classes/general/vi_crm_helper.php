...
class CVoxImplantCrmHelper
{
	...
	public static function AddCall(array $callFields)
	{
		...
		$activityFields['SUBJECT'] = self::createActivitySubject($callFields);
	...
	public static function RegisterEntity($params)
	{
		...
		$res = VI\CallTable::getList(array(
			'select' => array('*', 'PORTAL_NUMBER' => 'CONFIG.SEARCH_ID'),
		...
		else
		{
			...
			if ($history = $res->fetch())
			{
				...
				CVoxImplantCrmHelper::AddCall(Array(
					...
					'PORTAL_NUMBER' => $history['PORTAL_NUMBER']
				));
	...
	public static function createActivitySubject(array $callFields)
	{
		if($callFields['INCOMING'] == CVoxImplantMain::CALL_OUTGOING)
			return Loc::getMessage('VI_CRM_ACTIVITY_SUBJECT_OUTGOING', array('#NUMBER#' => $callFields['CALLER_ID']));
		else if($callFields['INCOMING'] == CVoxImplantMain::CALL_INCOMING || $callFields['INCOMING'] == CVoxImplantMain::CALL_INCOMING_REDIRECT)
			return Loc::getMessage('VI_CRM_ACTIVITY_SUBJECT_INCOMING', array('#NUMBER#' => $callFields['CALLER_ID']));
		else if($callFields['INCOMING'] == CVoxImplantMain::CALL_CALLBACK)
			return Loc::getMessage('VI_CRM_ACTIVITY_SUBJECT_CALLBACK', array('#NUMBER#' => $callFields['CALLER_ID']));
		else
	...
	public static function resolveEntitiesFields(array $entities)
	{
		...
		foreach ($entities as $entity)
		{
			...
			$resolvedEntity['SHOW_URL'] = CCrmOwnerType::GetShowUrl(CCrmOwnerType::ResolveID($entity['TYPE']), $entity['ID'], false);
