...
class Connector
{
	...
	public static function onMessageUpdate($messageId, $messageFields, $flags)
	{
		...
		)
		{
			...
			{
				$flags['TEXT'] = '[b]'.self::getOperatorName($lineId, $messageFields['AUTHOR_ID']).':[/b]'.(strlen($flags['TEXT'])>0? '[br] '.$flags['TEXT']: '');
			}
	...
	public static function onMessageSend($messageId, $messageFields)
	{
		...
		else
		{
			...
			if ($messageFields['SYSTEM'] != 'Y' && self::isEnableSendMessageWithSignature($connectorId) && $messageFields['AUTHOR_ID'] > 0)
			{
				$messageFields['MESSAGE'] = '[b]'.self::getOperatorName($lineId, $messageFields['AUTHOR_ID']).':[/b]'.(strlen($messageFields['MESSAGE'])>0? '[br] '.$messageFields['MESSAGE']: '');
			}
	...
	public static function getOperatorName($lineId, $userId, $array = false)
	{
		...
		if ($array)
		{
			...
			$userFields['firstName'] = $userArray['NAME'];
			$userFields['lastName'] = $userArray['LAST_NAME'];
			$userFields['name'] = $userName;
			$userFields['nameList'] = $userName;
