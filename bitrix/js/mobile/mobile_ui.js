;(function ()
{
	...
	window.BX.MobileUI = {
		...
		addCopyableDialog: function (node, textBlockClass, getTextFunction)
		{
			BX.MobileApp.Gesture.addLongTapListener(node, function ()
			{
				BX.addClass(node, "long-tap-activate");
					...
					buttons: [
						{
							...
							callback: function ()
							{
								...
								if (textBlockClass)
								{
									var copyableBlock = BX.findChild(node, {className: textBlockClass}, true);
									if (copyableBlock)
									{
				...
				setTimeout(function ()
				{
					BX.removeClass(node, "long-tap-activate");
				}, 1000);
			});
		},
		/**
